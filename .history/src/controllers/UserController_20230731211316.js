...
class UserController {
  ...
  async GetFeeShip(req, res) {
    ...
    GetServiceShip(1452, id_district).then((resData) => {
      const id_service = resData;
      GetFee(id_service, 1452, "21014", id_district, id_ward);
      res.send({ message: resData[0] });
    });
...
const GetServiceShip = (from_district, to_district) => {
  ...
  return axios
    ...
    .then((result) => result.data.data[0].service_id)
    .catch((error) => {
      console.log(error);
    });
};

const GetFee = (service_id, from_district, from_ward, to_district, to_ward) => {
  const shopid = process.env.GHN_SHOPID;
  const linkGHN = process.env.GHN_LINK;
  const token = process.env.GHN_TOKEN;
