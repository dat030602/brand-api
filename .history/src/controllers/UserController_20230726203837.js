...
class UserController {
  ...
  async GetFeeShip(req, res) {
    ...
    const resData = await execute.executeQuery(
      `SELECT TEN_TINH,TEN_THANHPHO,TEN_PHUONG FROM DIACHIGIAOHANG DC JOIN TINH T ON DC.TINH = T.MA_TINH JOIN THANHPHO TP ON DC.THANHPHO = TP.MA_THANHPHO JOIN PHUONG P ON DC.PHUONG = P.MA_PHUONG WHERE MA_KHACH = '${id_user}' AND STT = '${stt}'`
    );
    res.send(resData);
    const id_service = await GetServiceShip(1452, id_district);
  }

  async GetServiceShip(from_district, to_district) {
    const shopid = process.env.SHOPID_GHN;
    const linkGHN = process.env.LINK_GHN;
    const token = process.env.TOKEN_GHN;
    const baseUrl = `${linkGHN}/available-services?shop_id=${shop_id}&from_district=${from_district}&to_district=${to_district}`;
    let config = {
      headers: {
        token: token,
      },
    };
    axios.get(linkGHN, config).then((result) => {
      console.log(result);
    });
  }
}
