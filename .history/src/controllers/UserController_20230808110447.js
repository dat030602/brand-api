...
class UserController {
  ...
  async GetFeeShip(req, res) {
    ...
    // GetFee(id_service, 1452, "21014", id_district, id_ward).then(
    //   (feeShip) => {
    //     feeShip = Number((feeShip / 23745).toFixed(2));
    //     res.send({ message: feeShip });
    //   }
    // );
    // await GetFee(id_service, 1452, "21014", id_district, id_ward);
    // });
    // console.log(id_service);
    const id_service = await GetServiceShip(1452, id_district);
    const fee = await GetFee(id_service, 1452, "21014", id_district, id_ward);
    const date = await GetDate(id_service, 1452, "21014", id_district, id_ward);
    // console.log("temp: ", temp);
    res.send({ id_service, fee, date });
  }
}
