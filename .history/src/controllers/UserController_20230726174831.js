...
class UserController {
  ...
  async Address(req, res) {
    const id_user = req.query.id_user;
    const resData = await execute.executeQuery(
      `SELECT SONHA_DUONG,TEN_TINH,TEN_THANHPHO,TEN_PHUONG FROM DIACHIGIAOHANG DC JOIN TINH T ON DC.TINH = T.MA_TINH JOIN THANHPHO TP ON DC.THANHPHO = TP.MA_THANHPHO JOIN PHUONG P ON DC.PHUONG = P.MA_PHUONG WHERE MA_KHACK = ${id_user}`
    );
    res.send(resData);
  }
}
