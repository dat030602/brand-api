      ...
      if (data.typePayment === "paypal") {
        ...
        if (responeDataFromPayPal.status === "fail") return { status: "Error" };
          ...
          let result2 = await pool
            .request()
            .input("MA_KHACH", sql.VarChar(10), data.dataUser.id_user)
            .query("select MA_GH from GIOHANG WHERE MA_KHACH = @MA_KHACH");
          console.log(result2.recordset[0].MA_GH);
          const MA_DH = `DH${result.recordset[0].sl}`;
          const MA_GH = result2.recordset[0].MA_GH;
          let result3 = await pool
            .request()
            .input("MA_DONHANG", sql.VarChar(10), id_user)
            .input("MA_KHACH", sql.VarChar(10), id_user)
            .input("NGAY_TAO", sql.VarChar(10), id_user)
            .input("STT_DIACHI", sql.VarChar(10), id_user)
            .input("TONGTIEN", sql.VarChar(10), id_user)
            .input("PHI_GIAO_HANG", sql.VarChar(10), id_user)
            .input("LOAI_THANHTOAN", sql.VarChar(10), id_user)
            .input("THUE", sql.VarChar(10), id_user)
            .input("GIAM_GIA", sql.VarChar(10), id_user)
            .input("GIAM_GIA_GIAO_HANG", sql.VarChar(10), id_user)
            .input("LY_DO_HUY", sql.VarChar(10), id_user)
            .input("MA_GH", sql.VarChar(10), id_user)
